...
​	**/1.上电后是硬件将逻辑地址0x00和0x04分别放进sp和pc（具体的物理地址根据boot引脚的状态变化而硬件设置，如0x 进入flash的地址0x08000000和0x08000004），而无论物理起始地址是在哪里，都是指向向量表的起始地址，因此，0x00根据向量表的定义是sp指针的地址（启动文件里面栈区域和向量表之间还间隔堆，但是在链接文件处理之后，是在block1的sram，栈在高地址向下生长，堆在低地址向上生长），0x04根据向量表定义是Reset_Handler复位函数入口的地址，也就是说0x00和0x04逻辑上 就可以实现软件的选择开始运行的地址；**
