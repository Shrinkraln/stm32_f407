...
本章参考资料《STM32F4xx 中文参考手册》第十章-中断和事件；MDK中的帮助手册—ARM Development Tools：用来查询ARM的汇编指令和编译器相关的指令。
...
2. **初始化PC指针=Reset_Handler(纯粹由硬件电路实现)**
3. **初始化中断向量表**
4. **配置系统时钟**
...
![ARM汇编指令索引](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/_images/Startu002.png)
...
![启动文件使用的ARM汇编指令汇总](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/_images/Startu01.png)
...
#### **EQU是告诉编译器协助编译的，AREA到下一个AREA之前分配的地址空间都是该段的空间，启动文件只是先声明这些段的大小，具体位置是靠连接文件确定**
...
**标号__initial_sp紧挨着SPACE语句放置，表示栈的结束地址，即栈顶地址，栈是由高向低生长的（高地址叫做栈顶，但是不是生长方向的顶）**。
...
当内核响应了一个发生的异常后，对应的异常服务例程(ESR)就会执行。为了决定 ESR 的入口地址， 内核使用了“向量表查表机制”。这里使用一张向量表。 向量表其实是一个 WORD（ 32 位整数）数组，每个下标对应一种异常，该下标元素的值则是该 ESR的入口地址。向量表在地址空间中的位置是可以设置的， 通过 NVIC 中的一个重定位寄存器来指出向量表的地址。在复位后，该寄存器的值为 0。因此，在地址 0 （即FLASH 地址0）处必须包含一张向量表， 用于初始时的异常分配。要注意的是这里有个另类： 0 号类型并不是什么入口地址，而是给出了复位后MSP 的初值。

**在复位的过程中：**

​	**/1.上电后是硬件将物理地址0x00和0x04分别放进sp和pc（具体的物理地址根据boot引脚的状态变化而硬件设置，如0x 进入flash的地址0x08000000和0x08000004），而无论物理起始地址是在哪里，都是指向向量表的起始地址，因此，0x00根据向量表的定义是sp指针的地址（启动文件里面栈区域和向量表之间还间隔堆，但是在链接文件处理之后，是在block1的sram，栈在高地址向下生长，堆在低地址向上生长），0x04根据向量表定义是Reset_Handler复位函数入口的地址，也就是说0x00和0x04逻辑上 就可以实现软件的选择开始运行的地址；**

​	**/2.而在Reset_Handler里面，会修改VTOR寄存器的值为对应的向量表的地址（上电默认是物理上的0x00，修改为逻辑上的0x00），那么当异常触发的时候，就会根据硬件设计确定的异常的序号n，读取VTOR+n*4地址上的异常处理入口进入；**

​	**/3.对于IAP，是有两套向量表，app的中断由于此时是app的向量表，进入app中断，iap中断对应iap向量表（flash的逻辑0x00），进入iap中断，两者互不干扰，并且之后通过复位之后重新执行iap才可能“在触发app中断之后触发iap中断”**

![F407向量表](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/_images/Startu02.png)

代码清单:启动文件-1 F407向量表

|      | `__Vectors  DCD   __initial_sp        ;栈顶地址        DCD   Reset_Handler       ;复位程序地址        DCD   NMI_Handler        DCD   HardFault_Handler        DCD   MemManage_Handler        DCD   BusFault_Handler        DCD   UsageFault_Handler        DCD   0                    ; 0 表示保留        DCD   0        DCD   0        DCD   0        DCD   SVC_Handler        DCD   DebugMon_Handler        DCD   0        DCD   PendSV_Handler        DCD   SysTick_Handler  ;外部中断开始        DCD   WWDG_IRQHandler        DCD   PVD_IRQHandler        DCD   TAMP_STAMP_IRQHandler ;限于篇幅，中间代码省略        DCD   CRYP_IRQHandler        DCD   HASH_RNG_IRQHandler        DCD   FPU_IRQHandler __Vectors_End __Vectors_Size EQU __Vectors_End - __Vectors ` |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
![CM4内核的指令](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/_images/Startu03.png)
...
首先判断是否定义了__MICROLIB ，如果定义了这个宏则赋予标号__initial_sp（栈顶地址）、__heap_base（堆起始地址）、 __heap_limit（堆结束地址）全局属性，可供外部文件调用。有关这个宏我们在KEIL里面配置， 具体见图 [使用微库](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/book/Startup_file.html#id11) 。然后堆栈的初始化就由C库函数_main来完成。

![使用微库](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/_images/Startu003.png)

如果没有定义__MICROLIB，则插入标号__use_two_region_memory，这个函数需要用户自己实现，具体要实现成什么样， 可在KEIL的帮助文档里面查询到，具体见图 [use_two_region_memory函数](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/book/Startup_file.html#use-two-region-memory) 。

![__use_two_region_memory函数](https://doc.embedfire.com/mcu/stm32/f407batianhu/std/zh/latest/_images/Startu004.png)
